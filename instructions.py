...
class Instructions:
    ...
    def sanitise_instruction(r, m, ins):
        ...
        if rem_com_li[0] in j_format:
            ...
            for i in rem_com_li[1:-1:]:
                if i.startswith("$") and not r.get_register_validity(i[1::]):
    ...
    def lbu(r, m, reg, val):
        ...
        if "(" in val and ")" in val:
            ...
            # value is a value
            # store the value at the address
            r.set_register(reg[1::], abs(m.load_existing_unsigned_byte(val)))
    ...
    def lb(r, m, reg, val):
        ...
        if "(" in val and ")" in val:
            ...
            address = r.get_register(register[1:]) + offset
            # store the value at the address
            r.set_register(reg[1::], m.load_existing_byte(address))
            ...
            # value is a value
            # store the value at the address
            r.set_register(reg[1::], m.load_existing_byte(val))
